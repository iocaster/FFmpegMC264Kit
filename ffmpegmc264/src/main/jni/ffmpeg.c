...
static void ffmpeg_cleanup(int ret)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_cleanup() : Enter...\n");
    ...
    term_exit();
    ffmpeg_exited = 1;
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_cleanup() : Exit ...\n");
}
...
int ytkim_ffmpeg_main(int argc, char **argv)
{
    ...
    //set my own log function. start_logger() is still effective if JNI_OnLoad() is called at startup.
    ...
    int jmpret = setjmp(jumpBuff);  //jmpret = 0 at this time
    if( jmpret != 0 )
        goto ytkim_ffmpeg_jump_exit;
...
ytkim_ffmpeg_jump_exit:
    if( jmpret != 0 ) {
        main_return_code = jmpret;
        if( jmpret == 254 )             //254 set by cmdutil.c :: exit_program(ret) when ret == 0. So, get back 254 to 0.
            main_return_code = 0;
    }
    av_log(NULL, AV_LOG_DEBUG, "ffmpeg_main() : Exit ... jmpret = %d\n", jmpret); //jmpret = n, it's the value of longjmp(..,n) by exit_program()
